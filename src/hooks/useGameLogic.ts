...
import { GameStatus, Player, Position, PoisonPlacementStatus, ApiDebugEntry, DiceResult, PoisonHistoryEntry } from '@/types';
import { GameApiService } from '@/services/gameApi';

/**
 * 游戏逻辑自定义Hook - 封装所有游戏相关状态和逻辑
 */
export function useGameLogic(initialMapSize = 5) {
  // 游戏状态管理
  const [mapSize, setMapSize] = useState<number>(initialMapSize);
  ...
  const [apiDebugData, setApiDebugData] = useState<ApiDebugEntry[]>([]);
  const [showApiResponse, setShowApiResponse] = useState<boolean>(false);
  const [poisonHistory, setPoisonHistory] = useState<PoisonHistoryEntry[]>([]);
  const [currentRound, setCurrentRound] = useState<number>(1);
  
  // 创建API服务实例
  const [gameApi, setGameApi] = useState<GameApiService>(
    new GameApiService(apiKey, modelId)
  );
  
  // 当API密钥或模型ID变化时更新API服务
  useEffect(() => {
    gameApi.updateConfig(apiKey, modelId);
  }, [apiKey, modelId, gameApi]);
  ...
  const completePoisonPlacement = async () => {
    if (!playerPoison) return;
    
  // 获取系统毒药位置
   const systemPos = await gameApi.getSystemPoisonPosition(
     playerPoison, 
      mapSize,
      setApiDebugData,
      poisonHistory,
  ...
  // 处理API配置更改
  const handleApiConfigChange = (type: 'apiKey' | 'modelId', value: string) => {
      if (type === 'thinkingMode') {
        setThinkingMode(value as 'enabled' | 'disabled' | 'auto');
      } else if (type === 'apiKey') {
      setApiKey(value);
    } else {
      setModelId(value);
  ...
  return {
    ...
    toggleShowApiResponse,
    systemSelectPosition,
    poisonHistory
  };
}